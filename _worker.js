...
export default {
  async fetch(request, env) {
    ...
    if (!url.pathname.startsWith("/api/")) {
      const directAsset = await env.ASSETS.fetch(rewriteDirectAssetRequest(request));
      if (directAsset.status !== 404) {
        return directAsset;
      }

      return env.ASSETS.fetch(rewritePublicAssetRequest(request));
    }
...
function rewriteDirectAssetRequest(request) {
  const url = new URL(request.url);
  if (url.pathname === "/") {
...
function rewritePublicAssetRequest(request) {
  const url = new URL(request.url);
